...
import datetime, time, os, sys, requests, shutil, json

# e.g. ALL, DEBUG, INFO, NONE 
log_level='ALL'
...
def run_image_cleanup():
  ...
  log("run_image_cleanup(): >", "ALL");
  if not active cameras:
    # Do nothing, as there's no reason to do any image cleanup.  
  else:
    # We want [number_of_still_images_to_retain] total, so to get the number of images to retain per camera:
    number_of_still_images_to_retain_per_camera = number_of_still_images_to_retain / len(active_cameras)
    #for camera in active_cameras:
    #  number_of_still_images_currently_stored_for_this_camera = 
    # returnnumber_of_still_images_to_retain

def check_for_sumpple_camera_at_address(ip_address):
  log("check_for_sumpple_camera_at_address(): > ip_address=" + ip_address, "ALL");
  try: 
    ...
    if (camera_status_request.status_code == 200):
      log("check_for_sumpple_camera_at_address(): found camera at " + ip_address, "DEBUG");
      return True
  except:
      pass
  return False
...
def scan_network_for_cameras():
  global last_scan_for_active_cameras, active_cameras
  
  # TO DO: Instead of using a static list of IPs, accept a subnet mask and scan the subnet looking for cameras
  log("scan_network_for_cameras(): >", "ALL");
  active_cameras=[]
  for camera_address in camera_list:
    log("scan_network_for_cameras(): scanning camera at address: " + camera_address, "DEBUG")
  ...
  log("scan_network_for_cameras(): Setting last_scan_for_active_cameras to: " + str(last_scan_for_active_cameras), "DEBUG")
  
def pull_still_images_from_active_cameras():
  log("pull_still_images_from_active_cameras(): >", "DEBUG")
  try:
  ...
  for camera_address in active_cameras:
    ...
    log("pull_still_images_from_active_cameras(): Retrieving still image from " + camera_address + " via the FOSCAM 'snapshot' function", "INFO")
    ...
    if snapshot_request.status_code == 200:
      ...
      snapshot_metadata_filename=image_data_base_folder + "/" + camera_address + ".metadata"
      snapshot_metadata='{ "timestamp" : "' + image_display_timestamp + '"}'
      ...
      # Store the metadata
      with open(snapshot_metadata_filename, 'w') as snapshot_metadata_file:
        snapshot_metadata_file.write(snapshot_metadata)
...
def scan_loop():
  ...
  while True: 
    ...
    log("scan_loop(): last_scan_for_active_cameras="  + str(last_scan_for_active_cameras)  + 
                    " current_time=" + str(current_time) + 
                    " scan_for_active_cameras_polling_interval_in_seconds=" + str(scan_for_active_cameras_polling_interval_in_seconds),  
    "DEBUG")
    if (
...
def log(message, level="INFO"):
  if (level >= log_level):
    if level !=  "INFO":
      log_level_label=": " +  level[0]
    else:
      log_level_label=""
    print "SimpleSumpple: " + '{:%Y.%m.%d %H:%M:%S}'.format(datetime.datetime.now()) + log_level_label + ": " + message
